...
import { StyleSheet, FlatList, View, I18nManager, TouchableOpacity } from "react-native";
...
export default class SendScreen extends React.Component {
    ...
    componentDidMount = async () => {
        this.props.navigation.setOptions({
            ...
            headerLeft: () => (
                <TouchableOpacity style={styles.btnAdd} onPress={() => this.props.navigation.goBack()} >
    ...
    receive = async () => {
        ...
        if (details && details.length > 0) {
            ...
            global.toast.show(translate('msgNoItemsAdded'), { type: "danger" });
    }

    renderItem = ({ item, index }) => {
        return (
            <ItemDetails type={'receive'} item={item} delete={this.deleteItem} />
        )
    }
    ...
    renderFooter = () => {
        return (
            <View style={styles.footer}>
                <Button style={styles.btnFooter} text={translate('receive')} onPress={this.receive} />
            </View>
        )
    }
