...
import { ItemDetails, AddItemModal } from "components/orders";
...
export default class ProducedItemsScreen extends React.Component {
    ...
    componentDidMount = async () => {
        ...
        if (selectedOrders && selectedOrders.length > 0) {
            ...
            try {
                var response = await services.getOrderDetails(selectedOrders, CheckType.Checkout);
                if (response) {
                    details = response;
                }
            }
    ...
    setHeaderRight = (hide) => {
        if (!hide) {
            return (<TouchableOpacity style={styles.btnAdd} onPress={() => this.setState({ showModal: true })} >
        ...
        if (!this.state.isSending) {
            this.setState({ isSending: true }, async () => {
                ...
                if (order && details && details.length > 0) {
                    ...
                    var data = { actionId: order.actionID, checkType: CheckType.Checkout };
    ...
    renderItem = ({ item, index }) => {
        return (
            <ItemDetails item={item} actionId={this.state.actionId} delete={this.deleteItem} />
        )
    }
    ...
    renderFooter = () => {
        return (
            <View style={styles.footer}>
                <Button style={styles.btnFooter} text={translate('finish')} onPress={this.finish} />
            </View>
        )
    }
